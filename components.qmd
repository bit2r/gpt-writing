...
[\LaTeX](http://example.org)은 수식 뿐만 아니라 복잡하고 미려한 표도 코드를 통해 제작할 수
있다. 쿼토에서 [\LaTeX](http://example.org) 표를 제작하기 위해서는 먼저 표제작을 위한
[\LaTeX](http://example.org) 표코드를 준비하고, GitHub에서 팬독-필터
...
하는 학문으로 정확하고 명료한 수학적 표기는 매우 중요하다. [\LaTeX](http://example.org)은
수학적 표현을 깔끔하고 정확하게 표현할 수 있는 도구 중 하나로 [\LaTeX](http://example.org)를
이용해 다양한 수학적 표현법을 살펴보자.

[\LaTeX](http://example.org)은 기본적인 사칙연산에서부터 지수, 삼각함수, 적분, 행렬, 그리스
...
이 외에도 [\LaTeX](http://example.org)로 더 복잡한 수식표현도 얼마든지 가능하다.

## 참고문헌

[\LaTeX](http://example.org) 문서 컴파일 과정은 몇 단계에 걸쳐 이뤄진다. 먼저 `xelatex`
명령을 사용하여 원본 [\LaTeX](http://example.org) 코드를 컴파일하면 `.aux`와 `.log` 파일이
...
노트북, [\LaTeX](http://example.org) 문서에서 참고문헌을 관리하는 데 사용된다. `.bib` 파일은
다양한 참고문헌(예: 논문, 책, 인터넷 자료 등)에 대한 메타데이터를 담고
있는데 제목, 저자, 출판일 등의 정보가 담겨있다.

[\LaTeX](http://example.org)으로 작성한 `.tex` 소스파일을 `references.bib` 서지 파일과
...
다음 [\LaTeX](http://example.org) 저작 파일은 서지 데이터가 담긴 `references.bib` 파일에
`Kim2017`과 `xie2020r` 두 개의 참고문헌이 들어있다.

[\LaTeX](http://example.org) 문서는 `biblatex` 패키지를 사용하여 이 `references.bib` 파일을
불러오고, `\cite` 명령어로 특정된 참고문헌을 인용한다. 참고문헌 목록은
`\printbibliography[title=참고문헌]` 명령어 출력되는데 `title` 옵션으로
참고문헌목록 제목을 "References"에서 "참고문헌"으로 변경하고 [\LaTeX](http://example.org)
문서에서 인용된 참고문헌을 기본설정 `.csl`에 맞춰 출력한다. 만약, 특정
참고문헌 스타일을 반영하고자 한다면 [\LaTeX](http://example.org) 파일에 `.csl` 파일을
지정하면 된다.
...
한글로 전환하여 개발을 진행한다. 아래 [\LaTeX](http://example.org) 문서 예시에서 전체적인
...
변경하여 한글 텍스트로 국문 문서임을 명확히 구성한다. 둘째, [\LaTeX](http://example.org)에서
